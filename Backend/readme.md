...
Description
Authenticates a captain with the provided email and password. Returns an authentication token upon successful login.
...
Data Requirements and Validation
The /captains/login endpoint expects a JSON payload with the following structure:
...
Response Codes
200 OK: Captain successfully authenticated. Returns a JWT token and captain information.
400 Bad Request: Validation error or invalid input. The response body will contain an array of error messages, indicating which fields failed validation.
401 Unauthorized: Invalid email or password. The response body will contain an error message indicating that the email or password is incorrect.
...
Response Codes
200 OK: Successfully fetched captain profile information.
401 Unauthorized: Invalid or missing authentication token.
